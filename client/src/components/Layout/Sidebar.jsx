...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const getRoleMenuItems = (role) => {
    switch (role) {
      case 'Admin':
        return [
          { name: 'Dashboard', icon: FiGrid, path: '#', active: true },
          { name: 'User Management', icon: FiUsers, path: '#', locked: true },
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  <Icon size={16} className={item.active ? 'text-violet-400' : 'text-slate-500 group-hover:text-slate-300'} />
                  <span>{item.name}</span>
                </div>
                {item.locked && (
